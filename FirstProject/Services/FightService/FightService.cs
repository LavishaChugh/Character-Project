...
namespace FirstProject.Services.FightService
{
    public class FightService : IFightService
    {
        ...
        public async Task<ServiceResponse<FightResultDto>> Fight(FightRequestDto request)
        {
            ...
            try
            {
                var characters = await _context.Characters.Include(c => c.Weapon).Include(c => c.Skills).Where(c => request.CharacterIds.Contains(c.Id)).ToListAsync(); 
                ...
                while (!defeated)
                {
                    foreach(var attacker in characters)
                    {
                        ...
                        response.Data.Log.Add($"{attacker.Name} attacks {opponent.Name} with {attackUsed} with {(damage >=0 ? damage : 0)} damage");
